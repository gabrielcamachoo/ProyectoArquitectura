...
describe('progress-service', () => {
    ...
    it('serves first endpoint', async () => {
        const res = await (0, supertest_1.default)(app).get('/progress/student/:studentId/course/:courseId'.replace(':id', '1').replace(':studentId', 's').replace(':courseId', 'c').replace(':moduleId', 'm').replace(':userId', 'u'));
        expect(res.status).toBe(200);
